...
int main(void)
{
	...
	int i;
	XContrastadj_Config Contrastadj_Instance;
	/*
	...
	XGpio_DiscreteWrite(&cmos_rstn, 1, 0x1);
	usleep(500000);
	xil_printf("XGpio_DiscreteWrite continue\r\n");
	/*
	 * Initialize Sensor ÉãÏñÍ·
	 */
	sensor_init(&ps_i2c0);
	/*
	...
	if (XContrastadj_Initialize(&Contrastadj_Instance, XPAR_CONTRASTADJ_0_DEVICE_ID) != XST_SUCCESS)
	{
		xil_printf("XContrastadj_Initialize fail\n");
		return -1;
	}

	XContrastadj_EnableAutoRestart(&Contrastadj_Instance);
	XContrastadj_Start(&Contrastadj_Instance);

	xil_printf("XImg_filter_Start continue\r\n");
	...
	while(1)
	{
		for(int i=0;i<100;i++)
		{
			XContrastadj_Set_adj(&Contrastadj_Instance,50);
			usleep(500000);
		}
	}
	cleanup_platform();
